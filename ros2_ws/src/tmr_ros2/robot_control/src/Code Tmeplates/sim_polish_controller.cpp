#include "sim_polish_controller.h"
...
void robot_control::ruckig_move()
{
    ...
    if (ruckig_result != ruckig::Result::Finished) {
        ...
        if(ControlMode == WP2READY || ControlMode == WPWORK){
            ...
            if(ControlMode == WP2READY && PolishMode){
                exdata.time.clear();
                exdata.j.clear();
                exdata.jv.clear();
                start_time = now();
            ...
            //// Exxxxxxxxxxxxxxxxx data ////
            if(PolishMode){
                RCLCPP_INFO(get_logger(), "Polish is done.");
                OutPutEx2F();
            }
            //// Exxxxxxxxxxxxxxxxx data ////
...
void robot_control::cmd_callback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
    if(PolishMode) return;
...
{
    std::ifstream cFile("/home/timmy168/ros2_ws/exp_config.txt");
    if (!cFile.is_open()) {

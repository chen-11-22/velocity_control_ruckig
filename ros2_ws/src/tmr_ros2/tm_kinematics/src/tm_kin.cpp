...
#define TM900_DH_PARAMS
	...
	void forward_tool(const std::array<double, 6> q, Eigen::Matrix4d &T, double EElen) {
		...
		T(0,0)  = c1*sp*s6 - s1*s5*c6 + c1*cp*c5*c6;	T(0,1)  = c1*sp*c6 + s1*s5*s6 - c1*cp*c5*s6;	T(0,2)  = c1*cp*s5 + s1*c5;

		T(0,3)  = c1*(a2*c2 + a3*c2*c3 - a3*s2*s3) - d4*s1 + d6*c5*s1 + d5*c1*(c4*(c2*s3 + c3*s2) + s4*(c2*c3 - s2*s3)) + d6*c1*s5*(c4*(c2*c3 - s2*s3) - s4*(c2*s3 + c3*s2));

		T(1,0)  = s1*sp*s6 + c1*s5*c6 + s1*cp*c5*c6;	T(1,1)  = s1*sp*c6 - c1*s5*s6 - s1*cp*c5*s6;	T(1,2)  = s1*cp*s5 - c1*c5;

		T(1,3)  = s1*(a2*c2 + a3*c2*c3 - a3*s2*s3) + d4*c1 - d6*c1*c5 + d5*s1*(c4*(c2*s3 + c3*s2) + s4*(c2*c3 - s2*s3)) + d6*s1*s5*(c4*(c2*c3 - s2*s3) - s4*(c2*s3 + c3*s2));

		T(2,0)  = cp*s6 - sp*c5*c6;		T(2,1) = cp*c6 + sp*c5*s6;		T(2,2) = -sp*s5;

		T(2,3) = d1 - a2*s2 + d5*(c4*(c2*c3 - s2*s3) - s4*(c2*s3 + c3*s2)) - a3*c2*s3 - a3*c3*s2 - d6*s5*(c4*(c2*s3 + c3*s2) + s4*(c2*c3 - s2*s3));
		...
		//// TransEE2Flange //// 
		Eigen::Vector3d EE2Flange(0, 0, EElen);
		Eigen::Matrix4d EE2Fmatrix = Eigen::Matrix4d::Identity();
		EE2Fmatrix.block<3, 1>(0, 3) = EE2Flange.head<3>();

		T = T*EE2Fmatrix;
	}

	int inverse(const double* T, double* q_sols, const double* q_ref=nullptr) {
		int num_sols = 0;
